...
export function AccountSelector({ onChange, selectedAccounts: initialSelectedAccounts }: AccountSelectorProps) {
  ...
  // Initialize with all accounts if nothing is provided
  useEffect(() => {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="flex items-center justify-between w-[180px] h-9"
        >
          ...
          <CommandList>
            ...
            <CommandGroup heading="My accounts">
              {accounts.map((account) => (
                ...
                >
                  <Checkbox 
                    checked={selectedAccounts.accountIds.includes(account.account_id)}
