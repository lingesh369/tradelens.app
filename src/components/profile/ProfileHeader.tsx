...
import { TraderProfile } from "@/hooks/useCommunity";
...
export function ProfileHeader({
  ...
  const displayName = trader.first_name && trader.last_name
    ? `${trader.first_name} ${trader.last_name}`
    : trader.username || 'Unknown User';
  ...
  const actualStats = useMemo(() => {
    ...
    return {
      trades: trader.trades_count || 0,
      followers: trader.followers_count || 0,
      pnl: trader.total_pnl || 0,
      winRate: trader.win_rate || 0
    };
  }, [calculatedStats, trader]);
  ...
  const stats = [
    ...
    },
    {
      ...
      visible: !trader.stats_visibility || trader.stats_visibility.win_rate !== false,
  ...
  const activeSocialLinks = socialPlatforms.filter(platform =>
    trader.social_links?.[platform.key]
  );
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="hidden xl:flex items-start justify-between">
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-shrink-0">
                <Avatar className="h-20 w-20 lg:h-24 lg:w-24 ring-4 ring-primary/10 cursor-pointer">
                  <AvatarImage src={!imageError ? trader.avatar_url : undefined} onError={() => setImageError(true)} />
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {trader.badge && trader.badge !== 'Free' && <Badge variant="secondary" className="px-2 py-1">
                    {trader.badge}
                  </Badge>}
                </div>
                ...
                <div className="flex items-center gap-2">
                  {activeSocialLinks.length > 0 && activeSocialLinks.map(platform => <Button key={platform.key} variant="outline" size="sm" onClick={() => window.open(trader.social_links![platform.key], '_blank')} className="h-8 w-8 p-0">
            ...
            <div className="flex flex-col gap-3">
              ...
              {!isOwner && <div className="flex gap-2 my-[40px]">
                <Button variant={trader.is_followed_by_user ? "outline" : "default"} onClick={onFollow} disabled={isFollowLoading} className="min-w-[100px]">
                  {isFollowLoading ? 'Loading...' : trader.is_followed_by_user ? 'Unfollow' : 'Follow'}
                </Button>
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {trader.badge && trader.badge !== 'Free' && <Badge variant="secondary" className="px-2 py-1 text-xs">
                    {trader.badge}
                  </Badge>}
                </div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  {activeSocialLinks.length > 0 && activeSocialLinks.map(platform => <Button key={platform.key} variant="outline" size="sm" onClick={() => window.open(trader.social_links![platform.key], '_blank')} className="h-6 w-6 p-0">
              ...
              {!isOwner && (
                <>
                  ...
                  <div className="hidden lg:flex gap-2">
                    <Button variant={trader.is_followed_by_user ? "outline" : "default"} onClick={onFollow} disabled={isFollowLoading} className="min-w-[90px]">
                      {isFollowLoading ? 'Loading...' : trader.is_followed_by_user ? 'Unfollow' : 'Follow'}
                    </Button>
                    <Button variant="outline" onClick={onMessage}>
                      <MessageCircle className="h-4 w-4 mr-2" />
                  ...
                  <div className="flex lg:hidden gap-1">
                    <Button variant={trader.is_followed_by_user ? "outline" : "default"} onClick={onFollow} disabled={isFollowLoading} size="sm" className="px-3">
                      {trader.is_followed_by_user ? (
          ...
          <div className="md:hidden space-y-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {activeSocialLinks.length > 0 && (
                    <div className="flex items-center gap-1 mt-2">
                      {activeSocialLinks.map(platform => <Button key={platform.key} variant="outline" size="sm" onClick={() => window.open(trader.social_links![platform.key], '_blank')} className="h-6 w-6 p-0">
                        <span className="text-xs">{platform.icon}</span>
                      </Button>)}
                    </div>
                  )}
            ...
            {!isOwner && <div className="flex gap-2">
              <Button variant={trader.is_followed_by_user ? "outline" : "default"} onClick={onFollow} disabled={isFollowLoading} className="flex-1">
                {isFollowLoading ? 'Loading...' : trader.is_followed_by_user ? 'Unfollow' : 'Follow'}
              </Button>
